...
def test_build_export_dict_sanitizes_a_job_errors_embedded_home_path_and_raw_device_id(tmp_path, monkeypatch):
    ...
    home.mkdir()
    monkeypatch.setenv("USERPROFILE", str(home))
    monkeypatch.setenv("USERNAME", "SampleTestUser")
    ...
    job_errors = [{
        ...
        "error": (
            ...
            f"(device {raw_id}) -- see C--Users-SampleTestUser-scratch\\game.nsp"
        ),
        "finished_at": "t",
    }]
    ...
    assert "SampleTestUser" not in sanitized_error  # neither occurrence survives
