...
4. Confirm the restore. For a different target profile, type that profile's name exactly; the server checks it. The plan is one-use and expires. Immediately before the first write, the service rechecks the source and target. There is no automatic retry or Shell fallback.
